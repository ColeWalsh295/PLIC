...
```{r}
full.df <- full.df %>%
  mutate(Lab_level = factor(Lab_level, levels = c('HighSchool', 'Intro-Algebra',
...
add_summary(p, fun = 'mean_se', group = c('Lab_level')) +
  scale_color_manual(values = c('#1b9e77', '#d95f02', '#7570b3', '#e7298a', '#66a61e', 
                                '#e6ab02')) +
  scale_x_discrete(labels = paste(c('High school', 'Intro (algebra)', 'Intro (calculus)', 
                                    'Sophomore', 'Junior', 'Senior/graduate'), ' (N = ', 
                                  desc.tab$N, ')', sep ='')) +
  theme(axis.text.x = element_text(angle = 30, vjust = 1, hjust = 1),
        legend.position = "none") +
  labs(x = 'Course level', y = 'Average pretest score') +
  geom_label(data = desc.tab, aes(y = Avg + se + 0.2, label = paste(round(Avg, 2), 
                                                                    "\u00B1", round(se, 2))))
