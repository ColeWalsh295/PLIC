...
Models.MI.df <- sapply(Models.df, MI.vec.to.bits, df_Features = scores$models) %>%
  data.frame(.) %>%
  `colnames<-`(c("MI")) %>%
  mutate(Item = sapply(colnames(Models.df), function(x) toupper(x)),
        Prop.Sel = round(colMeans(Models.df), 3),
        Question = sapply(Item, function(x) toupper(substr(x, 1, 3)))) %>%
    select(Item, Question, Prop.Sel, MI) %>%
  arrange(desc(MI))

Models.MI.df.CI <- MI.CI(Models.df, scores$models, Models.MI.df, CI.Low = 0.025, CI.High = 0.975)

labels.list <- c('Q2B_9', 'Q2B_21', 'Q3B_9', 'Q3B_21', 'Q2B_11', 'Q3B_11', 'Q2B_8', 'Q3B_23')

ggplot(Models.MI.df.CI, aes(x = Prop.Sel, y = MI, color = Question, shape = Question)) +
  ...
  geom_errorbar(data = Models.MI.df.CI[Models.MI.df.CI[, 'Item'] %in% labels.list,], aes(ymin = CI.Low, ymax = CI.High), width = 0.01, size = 0.8, alpha = 1) +
...
**Models CI**
```{r}
MI.CI <- function(raw.dataframe, scores, out.dataframe, reps = 100, CI.Low = 0.05, CI.High = 0.95) {
...
Methods.MI.df.CI <- MI.CI(Methods.df, scores$methods, Methods.MI.df, CI.Low = 0.025, CI.High = 0.975)

Methods.MI.df
labels.list <- c('Q1D_61', 'Q1D_63', 'Q2D_35', 'Q2D_4', 'Q1D_3', 'Q2D_33', 'Q4B_4')

ggplot(Methods.MI.df.CI, aes(x = Prop.Sel, y = MI, color = Question, shape = Question)) +
  geom_point(size = 3.5, alpha = 0.25) +
  geom_errorbar(aes(ymin = CI.Low, ymax = CI.High), width = 0.01, size = 1, alpha = 0.25) +
  geom_point(data = Methods.MI.df[Methods.MI.df[, 'Item'] %in% labels.list,], aes(x = Prop.Sel, y = MI, color = Question), size = 3.5) +
  geom_errorbar(data = Methods.MI.df.CI[Methods.MI.df.CI[, 'Item'] %in% labels.list,], aes(ymin = CI.Low, ymax = CI.High), width = 0.01, size = 0.8, alpha = 1) +
...
Actions.MI.df.CI <- MI.CI(Actions.df, scores$actions, Actions.MI.df, CI.Low = 0.025, CI.High = 0.975)

Actions.MI.df
labels.list <- c('Q1E_1', 'Q1E_4', 'Q1E_13', 'Q2E_14', 'Q2E_6', 'Q3E_11', 'Q3E_13', 'Q3E_20')

ggplot(Actions.MI.df.CI, aes(x = Prop.Sel, y = MI, color = Question, shape = Question)) +
  geom_point(size = 3.5, alpha = 0.25) +
  geom_errorbar(aes(ymin = CI.Low, ymax = CI.High), width = 0.01, size = 1, alpha = 0.25) +
  geom_point(data = Actions.MI.df[Actions.MI.df[, 'Item'] %in% labels.list,], aes(x = Prop.Sel, y = MI, color = Question), size = 3.5) +
  geom_errorbar(data = Actions.MI.df.CI[Actions.MI.df.CI[, 'Item'] %in% labels.list,], aes(ymin = CI.Low, ymax = CI.High), width = 0.01, size = 0.8, alpha = 1) +
  scale_shape_manual(values = c(15, 16, 17)) +
