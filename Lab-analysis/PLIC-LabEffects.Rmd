...
mod.agency <- '
  ...
  level: 2
    agency =~ Q29_1 + Q29_2 + Q29_3 + Q29_4 + Q29_5 + Q28_1
...
mod.analysis <- '
  level: 1
    PostScores ~ PreScores
  level: 2
    analysis =~ Q32_1 + Q32_2

    analysis ~ Lab.goal.skills + Lab.goal.both

    PostScores ~ analysis + Lab.goal.skills + Lab.goal.both
'
...
fit <- sem(mod.analysis, data = df.matched, cluster = "Class_ID")
summary(fit, standardized = TRUE, fit.measures = TRUE, modindices = TRUE)
standardizedsolution(fit)
resid(fit, type = 'cor')
...
```{r}
df.FY[, names(df.FY) %like% "Class_ID|Q33|PreScores|PostScores"] %>%
  group_by(Class_ID, Q33_1, Q33_2, Q33_3, Q33_4) %>%
  summarize(avg.pre = mean(PreScores), avg.post = mean(PostScores), N = n()) %>%
  mutate(com = 1 * Q33_1 + 1.069 * Q33_2 + 1.935 * Q33_3 + 0.509 * Q33_4,
         avg.gain = avg.post - avg.pre) %>%
  arrange(desc(com)) %>%
  ungroup() %>%
  select(-c('Q33_2', 'Q33_3', 'Q33_4')) %>%
  ggplot(., aes(x = com, y = avg.gain)) +
  geom_point()
```


```{r}
df.FY <- unique(df.FY[, names(df.FY)[names(df.matched) %like% 
                              "Q28|Q29|Q31|Q32|Q33|Lab_purpose|Lab.goal.skills"]])

df.FY <- df.FY %>%
  filter(Lab_purpose != 'Mixed') %>%
  select(-Lab_purpose)
...
fit <- sem(mod.sem, data = df.BFY, cluster = "Class_ID")
summary(fit, standardized = TRUE, fit.measures = TRUE, modindices = TRUE)
standardizedsolution(fit)
resid(fit, type = 'cor')
```
