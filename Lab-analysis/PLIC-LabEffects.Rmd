...
mod <- lmer(PostScores ~ PreScores + Lab_level + Major + 
              Lab_purpose * (Gender + Race.ethnicity.AmInd + 
                               Race.ethnicity.Asian + Race.ethnicity.Black + 
...
labels = c("Constant", "Pretest", "Both-based", "Skills-based", "BFY", 
           "FY (calculus)", "Engineering", "Other", "Other Science", "Unknown", 
           "Non-binary", "Unknown",  "Woman", "American Indian", "Asian", 
...
df.matched[, names(df.matched) %like% "Q28|Q29|Q31"] <- 
  data.frame(lapply(df.matched[, names(df.matched) %like% "Q28|Q29|Q31"], 
                    function(x) droplevels(factor(as.vector(x), 
                                                  levels = c('1', '2', '3', '4', 
                                                             '5'), 
                                                  ordered = TRUE))))

df.matched[, names(df.matched) %like% "Q28|Q29|Q31"] <- data.frame(lapply(df.matched[, names(df.matched) %like% "Q28|Q29|Q31"],
