...
df.PLIC <- df.PLIC[!duplicated(df.PLIC$Class_ID),]

# Creates new gender/race/major columns
df.PLIC <- Collapse.vars(data.frame(df.PLIC)) %>%
...
df.PLIC[, names(df.PLIC) %like% "Q28|Q29|Q31|Q32|Q33"] <- 
  data.frame(lapply(df.PLIC[, names(df.PLIC) %like% "Q28|Q29|Q31|Q32|Q33"], 
                    function(x) droplevels(factor(as.vector(x), 
                                                  levels = c('1', '2', '3', '4',
                                                             '5'), 
                                                  ordered = TRUE))))

df.PLIC[, names(df.PLIC) %like% "Q28|Q29|Q31|Q32|Q33"] <- data.frame(lapply(df.PLIC[, names(df.PLIC) %like% "Q28|Q29|Q31|Q32|Q33"],                                                           function(x) as.numeric(x)))

df.PLIC <- df.PLIC[, names(df.PLIC) %like% "Q28|Q29|Q31|Q32|Q33"] %>%
  mutate(Assessment = 'PLIC')

df.ECLASS <- df.matched[!duplicated(df.matched$ResponseId.CIS),]

df.ECLASS <- df.ECLASS[, names(df.ECLASS) %like% "Q34|Q35|Q36|Q37|Q38"] %>%
  mutate(Assessment = 'ECLASS')

colnames(df.PLIC) <- colnames(df.ECLASS)
df.CIS <- rbind(df.ECLASS, df.PLIC)
df.CIS <- df.CIS[!duplicated(df.CIS[, names(df.CIS)[names(df.CIS) %like% "Q34|Q35|Q36|Q37|Q38"]]),]
...
mod.cfa <- '
  ...
  analysis =~ Q37_1 + Q37_2
  modeling =~ Q36_1 + Q36_2 + Q36_3 + Q36_4
  communication =~ Q38_1 + Q38_3 + Q38_4
  Q36_1 ~~ Q36_2 + Q36_3
  Q36_4 ~~ Q36_3 + Q36_2
  Q38_1 ~~ Q38_4
'
...
mod2 <- fa(df.CIS %>%
             select(-Assessment), nfactors = 2)
fa.diagram(mod2)

mod3 <- fa(df.CIS %>%
             select(-Assessment), nfactors = 3)
fa.diagram(mod3)

mod4 <- fa(df.CIS %>%
             select(-Assessment), nfactors = 4)
fa.diagram(mod4)

mod5 <- fa(df.CIS %>%
             select(-Assessment), nfactors = 5)
fa.diagram(mod5)
```
...
mod.sem <- '
  ...
  level: 2
    ...
    #analysis ~ Lab.goal.skills + Lab.goal.both
