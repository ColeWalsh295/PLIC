...
```{r}
df_full <- read.csv('C:/Users/Cole/Documents/DATA/PLIC_DATA/Collective_Surveys/Complete/Complete_Concat_CourseInfo.csv')
...
GetPrePostSurveys <- function(df, survey) {
  ...
  if(survey == 'PRE') {
    appendix <- 'x' # all presurvey columns have '_x' appended, postsurveys have '_y'
    df.survey <- df %>%
...
ggplot(Methods.MI.df, aes(x = Prop.Sel, y = MI, color = Question, shape = Question)) +
  ...
  geom_label_repel(data = subset(Methods.MI.df, Item %in% labels.list),
            ...
            nudge_y = 0.06, size = 6, show.legend = FALSE) +
  theme_classic() +
  theme(text = element_text(size = 18)) +
  labs(x = 'Fraction of times selected', y = 'Mutual information (bits)')
dev.off()
...
ggplot(net.df, aes(x = Prop.Sel, y = MI, color = Question, shape = Question)) +
  ...
  scale_color_manual(values = c(rep("#0072b2", 3), rep("#d55e00", 3), rep("#009e73", 3), 
                                "#cc79a7")) #+
  # scale_fill_manual(values = labels.list) +
  # geom_label_repel(data = subset(Actions.MI.df, Item %in% labels.list),
  #           aes(x = Prop.Sel, y = MI, color = Question, label = Item), nudge_x = 0.04, 
