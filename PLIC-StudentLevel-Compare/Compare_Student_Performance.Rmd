...
**LMER**
```{r}
mod_null <- lmer(PreScores_GrandMC ~ (1 | Class_ID), df.Centered)
summary(mod_null)
r.squaredGLMM(mod_null)

mod_student <- lmer(PreScores_GrandMC ~ Gender + Major + URM_Status + (1 | Class_ID), df.Centered)
summary(mod_student)
r.squaredGLMM(mod_student)

mod_course <- lmer(PreScores_GrandMC ~ Gender + Major + URM_Status + Lab_Level + (1 | Class_ID), df.Centered)
summary(mod_course)
r.squaredGLMM(mod_course)

mod_interaction <- lmer(PreScores_GrandMC ~ Gender*URM_Status + Major + Lab_Level + (1 | Class_ID), df.Centered)
summary(mod_interaction)
r.squaredGLMM(mod_interaction)

mod_interactionMajor <- lmer(PreScores_GrandMC ~ (Gender + URM_Status)*Major + Lab_Level + (1 | Class_ID), df.Centered)
