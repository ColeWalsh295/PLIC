...
def Individiual_Question_Network(df, Question, Survey = 'PRE'):
    ...
    for node, data in G.nodes(data = True):
        Node_Sizes_Array.append(data['# of Students'])
    ...
    nx.draw_networkx_edges(G, pos = pos, edge_cmap = plt.cm.Greys, edge_color = Edge_Colors_Array, edge_vmin = min(Edge_Colors_Array), edge_vmax = max(Edge_Colors_Array))
    plt.show()
...
def FindCommunities(df, Survey = 'PRE', Matched = False, Projection = 'Items', resolution = 1, Community_Nodes = False, Iterate = False, Sparsify = False, randomize = False, **kwargs):

    Infodf = pd.read_csv('C:/Users/Cole/Documents/GRA_Summer2018/Surveys/180202__PLIC__Working_version_3d_fix2.csv', nrows = 1).T.rename(columns = {0:'Description'})
    Others = ['Q1b_19', 'Q1d_10', 'Q1e_12', 'Q2b_38', 'Q2d_11', 'Q2e_11', 'Q3b_10', 'Q3d_29', 'Q3e_8', 'Q4b_11']
    ...
    df_total_Bipartite = pd.melt(df_total, id_vars = 'V1').dropna().reset_index(drop = True)[['V1', 'variable']].rename(columns = {'V1':'Student', 'variable':'item'})
    ...
    if(Projection == 'Items'):
        Communities2_df = pd.merge(left = Communities_df, right = Infodf, left_index = True, right_index = True)
        ...
        Communities2_df = pd.merge(left = Communities_df, right = df1, left_index = True, right_on = ID).sort_values(by = ['Community']).rename(columns = {ID:'Students'}).set_index('Students')

        nx.set_node_attributes(G, Node_Sizes, '# of Items in common')

    if((not Iterate) and (Projection == 'Items')):
        print('Modularity: {}'.format(community.modularity(Communities, G, weight = 'weight')))

        nx.set_node_attributes(G, Communities, 'Community')

        G1 = hv.Graph.from_networkx(G, nx.spring_layout, iterations = 50).options(width = 500, height = 500, color_index = 'Community', cmap = 'tab10', edge_color_index = 'weight', edge_cmap = 'Greys',
                                                                                    show_frame = False, xaxis = None, yaxis = None)

        D1 = community.generate_dendrogram(G, weight = 'weight', resolution = resolution)
...
def IterativeCommunityFinding(df, N_Iterations = 5, Community_Iters = 5, Cutoff = 0, Projection = 'Items', **kwargs):
        ...
        for c in range(Community_df['Community'].max() + 1):
            ...
            for t in Nodes_in_c:
                try:
                    ...
                    try:
                        Community_Overlap[tuple([t[1], t[0]])] += 1
                    except:
                        Community_Overlap[t] = 1
...
def SankeyItemTrees(Com_dfs = [], Labels = [], Projection = 'Items'):
    ...
    data = dict(
        ...
        node = dict(
          ...
          # color = [Colors[i] for c in range(Com_dfs[i].loc[:, 'Community'].max() + 1) for i in range(len(Com_dfs))]
...
def StudentProfiles(Student_df, Community_df = None, Survey = 'PRE', Compute = True, Iterate = False, **kwargs):
    ...
    if Compute:
        ...
        if Iterate:
            ...
            Community_df, O1_df, G1 = IterativeCommunityFinding(Student_df, Survey = Survey, Projection = 'Items', **kwargs)
            # Student_df, O2_df, G2 = IterativeCommunityFinding(df, Projection = 'Students', **kwargs)

        else:

            Community_df, G1, D = FindCommunities(Student_df, Survey = Survey, Projection = 'Items', **kwargs)

        Student_df, G2 = FindCommunities(Student_df, Survey = Survey, Projection = 'Students', **kwargs)

    Coms_array = Community_df.reset_index().rename(columns = {'index':'Items'}).groupby('Community')['Items'].apply(list).values
