...
mod1 <- lmer(PostScores ~ PreScores + Lab_purpose + Lab_level + Major + Gender + 
               ...
               Race.ethnicity.Hispanic + Race.ethnicity.NatHawaii + Race.ethnicity.White +
               Race.ethnicity.Other + (1 | anon_institution_id/Class_ID), df.matched)
summary(mod1)
...
stargazer(mod1, mod1, mod1, mod1, coef = list(summary(mod1)$coefficients[, 1], 
                                              summary(mod1)$coefficients[, 2], 
                                              mod1.std$coefficients[, 1], 
                                              mod1.std$coefficients[, 2]), 
...
ggplot(df.demos, aes(x = factor(group, levels = c('Man', 'Non-binary', 'Woman', 
  ...
  scale_color_manual(values = c('#e69f00', '#009e74', '#0071b2')) +
  labs(x = '', y = 'Predicted PLIC posttest scores', color = 'Lab purpose') +
...
df.matched <- df.matched %>%
  mutate(Lab.goal.skills = 1 * (Lab_purpose == 'Skills-based'),
         Lab.goal.both = 1 * (Lab_purpose == 'Both-based'),
...
mod.sem <- '
  ...
  level: 2
    decision =~ 1.0 * Q29_1 + 1.318 * Q29_2 + 1.205 * Q29_3 + 1.019 * Q29_4 + 0.958 * Q29_5 + 0.982 * Q31_6
    ...
    PostScores ~ decision + epistemic + Lab.goal.skills + Lab.goal.both
'
