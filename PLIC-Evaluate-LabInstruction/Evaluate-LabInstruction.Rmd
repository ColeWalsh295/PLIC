...
mod.main <- lmer(Gain ~ PreScores + Gender + URM_Status + Major + Lab_Purpose + (1 | School/Class_ID), df.Centered)
summary(mod.main)
r.squaredGLMM(mod.main)

mod.interaction <- lmer(Gain ~ PreScores + (Gender + URM_Status) * Lab_Purpose + Major + (1 | School/Class_ID), df.Centered)
...
beta(mod.main, skip = c('Gender', 'URM_Status', 'Major', 'Lab_Purpose'))
...
p1 <- plot_model(mod.main, type = 'eff', terms = 'Lab_Purpose', dot.size = 4, line.size = 1, ci.lvl = 0.67, title = '', axis.title = 'Predicted gain in PLIC scores', colors = c('#e69f00', '#009e74', '#0071b2'), dodge = 0.5) +
  ...
  scale_x_discrete(limits = c("Reinforce concepts", "Both about equally", "Develop skills"))

p2 <- p1
p2$data$x <- c(1, 2, 3)
p2
# ggplot(p2$data, aes(x = x, y = predicted)) +
#   geom_point(size = 4) +
#   geom_errorbar(aes(ymin = conf.low, ymax = conf.high), size = 1, width = 0.2) +
#   labs(x = 'Lab Purpose', y = 'Predicted gain in PLIC score') +
#   scale_x_discrete(limits = c("Reinforce physics concepts", "Both about equally", "Develop lab skills"))

#p1 <- plot_model(mod.interaction, type = 'eff', terms = c('Gender [Men, Women]', 'Lab_Purpose'), dot.size = 4, line.size = 1, ci.lvl = 0.67, title = '', axis.title = 'Predicted gain in PLIC scores', colors = c('#e69f00', '#009e74', '#0071b2'), dodge = 0.5) +
#  theme(legend.position = 'right')

p1 <- plot_model(mod.interaction, type = 'eff', terms = c('URM_Status', 'Lab_Purpose'), dot.size = 4, line.size = 1, ci.lvl = 0.67, title = '', axis.title = 'Predicted gain in PLIC scores', colors = c('#e69f00', '#009e74', '#0071b2'), dodge = 0.5) +
  theme(legend.position = 'right')
...
mod.ext <- '
  ...
  # intercepts
  Q29_1 ~ 0; Q29_2 ~ 0; Q29_3 ~ 0; Q29_4 ~ 0; Q29_5 ~ 0; agency ~ 0
  Q31_6 ~ 0; Q31_2 ~ 0; Q31_1 ~ 0; Q31_4 ~ 0; Q31_3 ~ 0; Q31_5 ~ 0; modeling ~ 0
...
df.Centered[, names(df.Centered) %like% "Q28|Q29|Q31"] <- data.frame(lapply(df.Centered[, names(df.Centered) %like% "Q28|Q29|Q31"], function(x) factor(as.vector(x), levels = c('1', '2', '3', '4', '5'), ordered = TRUE)))
...
cor(df.SEM[!duplicated(df.SEM$Class_ID), c('agency', 'PurposeSkills', 'PurposeBoth')])

mod.SEM <- '
  Gain ~ GenW + URM_StatusURM + MajorEng + MajorPhys + PurposeSkills + PurposeBoth + agency
  agency ~ PurposeSkills + PurposeBoth
  modeling ~ PurposeSkills + PurposeBoth
'
...
mod.SEM <- '
  ...
  PostScores ~ PreScores + GenW + GenNon + URM_StatusURM + MajorEng + MajorPhys + PurposeSkills + PurposeBoth + agency 
  agency ~ PurposeSkills + PurposeBoth
  # modeling ~ PurposeSkills + PurposeBoth
'

# df.Centered[, names(df.Centered) %like% "Q28|Q29|Q31"] <- data.frame(lapply(df.Centered[, names(df.Centered) %like% "Q28|Q29|Q31"], function(x) as.numeric(as.vector(x))))
...
mod.SEM2 <- '
  ...
  Gain ~ PreScores + GenW + URM_StatusURM + MajorEng + MajorPhys + PurposeSkills + PurposeBoth
  # agency ~ PurposeSkills + PurposeBoth
  # modeling ~ PurposeSkills + PurposeBoth
'
