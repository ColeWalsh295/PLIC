...
table(df.matched[!duplicated(df.matched$ResponseId),]$Lab.level, exclude = NULL)
table(df.matched[!duplicated(df.matched$ResponseId),]$Lab.goal, exclude = NULL)

colSums(df.matched[, c('Q52_1', 'Q52_5')], na.rm = TRUE)
```
...
chisq.test(df.matched[!duplicated(df.matched$ResponseId), 'Lab.goal'], 
           df.matched[!duplicated(df.matched$ResponseId), 'Lab.level'])
...
```{r}
mod0 <- lmer(student.score.post ~ (1 | ResponseId), df.matched)
r2(mod0)
...
class(mod1) <- "lmerMod"
class(mod2) <- "lmerMod"
beta(mod1, skip = noStandard.cols)
beta(mod2, skip = noStandard.cols)
```
